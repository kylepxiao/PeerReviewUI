...
source_file = 'pair_extraction/data/ReviewRebuttalPredict.txt'
...
def update_file1():
	with open('pair_extraction/data/file1new.csv', 'w') as outputfile:
		outputfile = csv.writer(outputfile)
		with open('pair_extraction/data/iclr_pairs (updated).csv') as iclr_pairs:
			iclr_pairs = csv.reader(iclr_pairs, delimiter=',')
			iclr_pairs = list(iclr_pairs)

			with open('pair_extraction/data/file1.csv') as csvfile1:
...
def main():
	with open(source_file, 'w') as outputfile:

		with open('pair_extraction/data/file2.csv') as csvfile2:
			file2 = csv.reader(csvfile2, delimiter=',')
			file2=list(file2)

			with open('pair_extraction/data/file1new.csv') as csvfile1:
				file1 = csv.reader(csvfile1, delimiter=',')
				...
				# f_vecs=open('pair_extraction/data/vecs','wb')
...
def sep_data():
	with open(source_file, "r") as full_data:
		...
		dev = open('pair_extraction/data/dev.txt', 'w')
		vecs = []
		for i in dev_data:
			dev.write(i + '\n\n')
			vec = calc_vec(i)
			...
			# vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		dev_vecs = open('pair_extraction/data/vec_dev.pkl', 'wb')
		pickle.dump(vecs, dev_vecs)
		dev_vecs.close()
		dev.close()
		print('dev done')"""

		print('processing predict vec file ...')
		test = open('pair_extraction/data/predict.txt', 'w')
		vecs = []
		for i in test_data:
			if i == '\n':
				continue
			...
			# vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		test_vecs = open('pair_extraction/data/vec_pred.pkl', 'wb')
		pickle.dump(vecs, test_vecs)
		test_vecs.close()
		test.close()
		print('predict embeddings done')

		"""print('processing train vec file ...')
		train = open('pair_extraction/data/train.txt', 'w')
		vecs = []
		for i in train_data:
			try:
				vec = calc_vec(i)
			...
			# vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		train_vecs = open('pair_extraction/data/vec_train.pkl', 'wb')
...
def generate_testnew():
	with open("test.txt", "r") as f:
		f = f.read().split('\n\n')
		fnew = open('pair_extraction/data/testnew.txt','w')
...
def get_paper_id():
	fnew=open(source_file,'r').readlines()
	fold=open('pair_extraction/data/ReviewRebuttalnew.txt','r').readlines()
	fout = open('pair_extraction/data/RR.txt','w')
...
def combine():
	fnew=open('pair_extraction/data/train1.txt','r').readlines()
	fold=open('pair_extraction/data/train.txt','r').readlines()
	fout = open('pair_extraction/data/checkI2B.txt','w')
	count=0
...
def match_diff_level():
    file_seg = open(source_file,'r').read().split('\n\n')
    output = open('pair_extraction/data/ReviewRebuttalnew3.txt','w')

    # file1 = csv.reader(open('pair_extraction/data/file1.csv'), delimiter=',')
...
def add_id():
	file1 = open('pair_extraction/data/RR.txt','r').readlines()
	csvfile2 = open('pair_extraction/data/file2.csv')
	file2 = csv.reader(csvfile2, delimiter=',')
	file2 = list(file2)
	output = open('pair_extraction/data/ReviewRebuttalnew3.txt','w')
	id = 0
...
def update_label():
	with open(source_file, 'w') as outputfile:
		...
		original =open('pair_extraction/data/ReviewRebuttalnew3.txt','r').readlines()
		flag=0
		next_flag=0
...
def match_train():
	file_all = open(source_file,'r').readlines()
	file_part = open('pair_extraction/data/train.txt','r').readlines()
	file_new = open('pair_extraction/data/train1.txt','w')
	for i in file_part:
		if i!='\n':
			for j in file_all:
...
def match_train_fast():
	file_all = open(source_file,'r').readlines() # confirm correct
	file_old = open('pair_extraction/data/test.txt','r').readlines() # Old, might be wrong
	file_new = open('pair_extraction/data/test1.txt','w') # follow the same order, but the content is from file_all
