...
class Reader:
    ...
    def read_txt(self, file: str, number: int = 5) -> List[Instance]:
        ...
        vec_path = list(os.path.split(file))
        vec_path[-1] = "vec_" + vec_path[-1][:4] + '.pkl'
        #f_vec = open(file[:8] + 'vec_' + file[8:-4] + '.pkl', 'rb')
        #print(file[:8] + 'vec_' + file[8:-4] + '.pkl')
        f_vec = open(os.path.join(*vec_path), 'rb')
        print(os.path.join(*vec_path))
